 ...
 * with optional memory caching for smaller APK entries.
 */
class XapkTestHandler4(private val context: Context) {
    ...
    fun runTest(xapkFileOnDisk: File, deviceConfig: DeviceConfig, appIconSize: Int, useMemoryCache: Boolean, preferApacheApiWhenPossible: Boolean = true): ApkParsingResult? {
        Log.d("AppLog", "XAPK Test 4: Started. useMemoryCache:$useMemoryCache")
            ...
            if (useApacheApi) {
                ...
                xapk.use { xapkFile ->
                    ...
                    if (baseApkEntry == null || packageName == null) {
                        Log.e("AppLog", "XAPK Test 4: Failed to find base APK")
                        return null
                    }
...
//                Log.w("AppLog", "XAPK Test 4: Fast path failed, using slow path")
                java.io.FileInputStream(xapkFileOnDisk).use { fis ->
                    ZipInputStream(fis).use { zis ->
                        ...
                        if (baseApkName == null || packageName == null) {
                            Log.e("AppLog", "XAPK Test 4 (Slow): Failed to find base APK")
                            return null
                        }
            ...
            Log.e("AppLog", "XAPK Test 4: Error", e)
        }
        return result
    }
